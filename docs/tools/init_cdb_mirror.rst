...
Kennzeichnend für die Vorgehensweise ist der Umfang der Instanz, welcher im SCM
System gehalten wird.  Bei der Vorgehensweise *"Software + Pakete
inkl. Kundenpaket installieren"* würde nur das zu ändernde (Kunden-) Paket im
SCM versioniert werden.
...
aus der Clone-Kopie dieses Ordners und dem DB Export auf.  Um dieses Verfahren
zu etablieren, werden keine CDB-Tools benötigt.  Die CDB-Tools können aber den
Aufbau des Spiegels erheblich erleichtern.
...
befinden sich die SQLite_ Datenbank ``filestore.db`` in welcher der BlobStore
